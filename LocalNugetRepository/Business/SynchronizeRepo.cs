...
namespace LocalNugetRepository.Business
{
    public class SynchronizeLocalRepository
    {
        ...
        {
            var result = newPackages.Where(x => IsFolderShouldBeInLocalNugetRepository(x.FullPath) && alreadyStoredPackages.All(y => y.Name != x.Name));
            return result;
        }
        ...
        public bool IsFolderShouldBeInLocalNugetRepository(string folder)
        {
            return HasNugetfolderVersionNumber(folder) || IsBeta(folder);
        }

        private bool HasNugetfolderVersionNumber(string folder)
        {
        ...
        private bool IsBeta(string folder)
        {
            if (string.IsNullOrEmpty(folder)) return false;
        ...
        public IEnumerable<string> GetFoldersToDelete(IEnumerable<string> folders)
        {
            ...
            foreach (var folder in folders)
            {
                try
                {
                    if (IsFolderShouldBeInLocalNugetRepository(folder)) continue;
